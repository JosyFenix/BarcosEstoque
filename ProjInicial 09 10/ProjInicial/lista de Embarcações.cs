...
namespace ProjInicial
{
    public partial class lista_de_Embarcações : Form
    {
        ...
        private void CampoPesquisa_TextChanged(object sender, EventArgs e)
        {
            ...
            if (comboBox1.Text == "descrBarcos")
            {
                meuAdapter = new MySqlDataAdapter("SELECT idBarcos, descrBarcos, barcoslocal from barcos WHERE NOME LIKE'" + CampoPesquisa.Text + "%'", mConnB);
                date = new DataTable();
